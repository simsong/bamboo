...
class FoggyCam(object):
  ...
  def now_time(form='%Y-%m-%d %H:%M:%S'):
    return datetime.datetime.now(datetime.UTC).strftime(form)
      ...
      if status:
        ...
        if resp.status_code == 200:
          ...
          if resp.status_code == 404:
            print(f"<> WARNING: {self.now_time()} {camera_name}: camera is offline. Waiting for {self.cam_retry_wait} seconds.")
          ...
          # Renew auth token
            ...
            sleep_time = 30
            print(f"<> DEBUG: {self.now_time()} {camera_name}: '{resp.reason}' ... failure received sleeping for {sleep_time} seconds.")
